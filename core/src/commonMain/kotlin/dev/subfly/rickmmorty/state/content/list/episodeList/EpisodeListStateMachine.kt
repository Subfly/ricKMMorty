...
class EpisodeListStateMachine(
    scope: CoroutineScope? = null
) : BaseStateMachine<EpisodeListEvent>(coroutineScope = scope) {

    private val getAllCharactersUseCase: GetAllEpisodes by inject()
    ...
    override fun onEvent(event: EpisodeListEvent) {
        when(event) {
            EpisodeListEvent.LoadMore -> handleLoadMoreEvent()
        }
    }
